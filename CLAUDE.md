...
Siehe `docs/` für alle Details: PRD.md, ARCHITECTURE.md, DATABASE.md, UI_UX.md, CHALLENGE_SYSTEM.md, BADGES_AND_RANKS.md, DEPLOYMENT.md.

### Netzwerkarchitektur (final)
- ESP32-S3 N16R8 = **nur** WLAN Access Point/DHCP, `192.168.4.1`, läuft über bestehendes `esp32_nat_router`-Projekt, unverändert. Kein DNS, kein Captive Portal, kein Webserver.
- Rock 4C+ = `192.168.4.2`, übernimmt dnsmasq/DNS, Captive Portal, FastAPI, React PWA, SQLite, Storage.
- Hostnamen `camp.local`, `helmpflicht.local`, `hub.local` zeigen alle auf `192.168.4.2`.
- Speicher: USB-SSD unter `/mnt/festival-data/` (uploads, thumbnails, exports, backups, database).
